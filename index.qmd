...
[**Second derivative test:**]{.ubdrubydeep} Measure the **change in slope** around a point $x$, i.e. $f''(\hat x) = \frac{d}{dx}\left( \frac{df}{dx}(x) \right) = \frac{d^2f}{dx^2}(x)$.
...
# A statistical perspective {.transition-slide-ubdrubydeep}
...
::: {.fragment}
<br>
The value $\hat \theta$ which maximises $\mathcal L(\theta)$ is called the [**maximum likelihood estimator**]{.ubdrubydeep} (MLE) of $\theta$.
:::
:::
...
::: aside
⚠️  Everything we need to know about the distribution is captured by the parameter $\theta$.
::: 
...
res |>
  ...
  geom_point(data = avgdots_df, aes(x, y), shape = "|", col = "red3") +
  # geom_vline(xintercept = 8, linetype = "dashed") +
  scale_x_continuous(
...
> 🧠 INTUITION: Stronger curvature → Easier to find optima → More information about the parameter → Less uncertainty (and vice versa)
...
```{r}
set.seed(136)
x <- rexp(50, rate = 1 / 300)

formatted <- formatC(round(x, 1), format = "f", digits = 1, width = 6)
mat <- matrix(formatted, nrow = 5, byrow = TRUE)
mat <- cbind(rep("      ", 5), mat)
invisible(apply(mat, 1, function(row) cat(paste(row, collapse = " "), "\n")))
```
...
$$
\hat\lambda = \frac{1}{\bar X} = \frac{1}{`r iprior::dec_plac(mean(x), 1)`} \approx `r scales::scientific(1 / mean(x))`.
$$
In other words, approximately one failure every `r round(mean(x))` hours.
...
\definecolor{ubdrubydeep}{HTML}{b71757}
...
\node[obsvar] (fx) [] {$\hspace{1em}f(x\mid\theta)\hspace{1em}$};
\node (xx) [right=of fx] {\textcolor{ubdblue}{$\{X_1,\dots,X_n\}$}};
\node (theta) [left=of fx] {\textcolor{ubdrubydeep}{$\theta$}};
\node (d1) [below=of fx,yshift=9mm] {Model};
\node (d2) [below=of xx,yshift=11mm] {\scriptsize \textcolor{ubdblue}{prob.}};
\node (d3) [below=of theta,yshift=11mm] {\scriptsize \textcolor{ubdrubydeep}{param.}};
...
\tikzstyle{connect}=[-latex, thick]
\node[obsvar] (fx) [] {$\hspace{1em}f(x\mid\theta)\hspace{1em}$};
\node (xx) [right=of fx] {\textcolor{ubdrubydeep}{$\{x_1,\dots,x_n\}$}};
\node (theta) [left=of fx] {\textcolor{ubdblue}{$\hat\theta$}};
\node (d1) [below=of fx,yshift=9mm] {Model};
\node (d2) [below=of xx,yshift=11mm] {\scriptsize \textcolor{ubdrubydeep}{data}};
\node (d3) [below=of theta,yshift=11mm] {\scriptsize \textcolor{ubdblue}{est.}};
...
::: {.column width=50%}
🌐 **Numerical derivatives** — how computers approximate calculus
...
🧠 **Bimodal and non-standard distributions**  when simple models break
:::
:::
:::

::: {.fragment fragment-index=1}
::: {.absolute right=-310 bottom=40}
```{r mleunique}
#| out-width: 68%
#| fig-height: 3
#| fig-width: 5
