...
The curvature $\kappa$ of a graph of a function $f$ at a point $x$ is defined as $\kappa = \frac{1}{r}$.
:::
...
res <-
  ...
  mutate(
    ...
    n = factor(n, labels = c("High curvature", "Low curvature"))
  )
...
circle <- tibble(
  n = factor(1:2, labels = c("High curvature", "Low curvature")),  
  xc = c(8, 8),
  a = c(r1, r2),
  b = 500 * c(1, 1 * r2 / r1),
  yc = tmp[1] - b,
...
res |>
  ...
  geom_text(data = circle_text, aes((xc + x) / 2 + 0.5, (yc + y) / 2 - 20, label = label), 
