...
## Combining hypothetical likelihoods

```{r}
B <- 100
X <- rbinom(B, size = 10, prob = 0.7)
xx <- seq(0.5, 0.9, length.out = 100)
tibble(
  b = seq_len(B),
  X = X,
  x = list(xx)
) |>
  mutate(L = map(X, \(x) dbinom(x, size = 10, prob = xx))) |>
  unnest(x, L) |>
  ggplot(aes(x, L, group = b)) +
  geom_line(alpha = 0.1) +
  theme_minimal()
```
